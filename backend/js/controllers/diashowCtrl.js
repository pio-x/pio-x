backendApp.controller('diashowCtrl', function($scope, $interval, apiService, teamService, logService, configService, stationService, mrxService){
    ...
    $scope.refresh = function() {
        if ($scope.diashowData[$scope.diashowIndex + 1]) {
            $scope.diashowIndex++;
            $scope.backgroundStyle['background-image'] = 'url("../api/image/' + $scope.diashowData[$scope.diashowIndex].img_ID + '.jpg?hash=' + $scope.hash + '")';
            $scope.caption = $scope.diashowData[$scope.diashowIndex].text;

            // cache next image
            if ($scope.diashowData[$scope.diashowIndex + 1]) {
                preloadImage('../api/image/' + $scope.diashowData[$scope.diashowIndex + 1].img_ID + '.jpg?hash=' + $scope.hash);
            }
        }
    };
